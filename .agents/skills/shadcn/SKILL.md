...
WailsのReact frontendで使用する。追加前に`frontend/components.json`と既存`frontend/src/components/ui`を確認し、画面設計・HTMLプロトタイプに必要な部品だけを導入する。Docker・ACP・Wails bindingの業務処理をUI部品へ置かない。
