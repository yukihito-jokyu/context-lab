...
各担当への依頼には、Issue番号、実装契約、所有範囲、禁止事項、必要な検証、他担当へ渡す情報だけを含める。担当結果を統合した後、独立レビューへ渡す。レビュー後はオーケストレーターが指摘を分類し、該当担当へ修正を委譲して再検証する。[review-gates.md](references/review-gates.md)を読む。
