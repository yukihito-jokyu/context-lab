...
func (s *Store) BeginExperimentBriefing(ctx context.Context, requestID string) (domain.ExperimentBriefingStart, bool, error) {
	...
	if _, err := tx.ExecContext(ctx, "INSERT INTO preparation_sessions (id, kind, state, created_at) VALUES (?, ?, ?, ?)", start.BriefingSessionID, "experiment_brief", start.State, time.Now().UTC().Format(time.RFC3339Nano)); err != nil {
		_ = tx.Rollback()
...
func (s *Store) updateExperimentBriefing(ctx context.Context, requestID, state, failureCode string) error {
	...
	sessionResult, err := tx.ExecContext(ctx, "UPDATE preparation_sessions SET state = ? WHERE id = (SELECT preparation_session_id FROM briefing_operations WHERE request_id = ?)", state, requestID)
	if err != nil {
		_ = tx.Rollback()
