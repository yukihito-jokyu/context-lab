...
func TestIsSQLiteBusy(t *testing.T) {
	tests := []struct {
		...
		{
			name:     "同時transaction開始",
			err:      errors.New("SQL logic error: cannot start a transaction within a transaction (1)"),
			wantBusy: true,
		},
		{
			name:     "永続化失敗",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if gotBusy := isSQLiteBusy(tt.err); gotBusy != tt.wantBusy {
				t.Errorf("isSQLiteBusy() = %v, want %v", gotBusy, tt.wantBusy)
			}
		})
	}
}
